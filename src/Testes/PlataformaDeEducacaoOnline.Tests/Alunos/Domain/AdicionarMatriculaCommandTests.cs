...
{   
    ...
    public Task AdicionarMatricula_CommandValido_DeveSalvarComSucesso()
    {
        // Arrange
        var command = new AdicionarMatriculaCommand(Guid.NewGuid(), Guid.NewGuid());
    ...
    public async Task ConcluirMatricula_MatriculaEncontrada_DeveSalvarComSucesso()
    {
        ...
        _mocker.GetMock<IStatusMatriculaRepository>().Setup(s => s.ObterPorCodigo((int)EnumMatricula.Concluida))
        ...
        _mocker.GetMock<IStatusMatriculaRepository>().Verify(s => s.ObterPorCodigo((int)EnumMatricula.Concluida), Times.Once);
